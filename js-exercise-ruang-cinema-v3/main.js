...
function findTicketAvailability(user, movie) {
...
function findRecommendation(user) {
    ...
    return recommendedMovies.filter(movie => findTicketAvailability(user, movie));
}
...
function generateRecommendation(user) {
    ...
    return recommendations.map(movie => ({
      ...
      name: movie.name,
      genre: movie.genre,
      totalPrice: movie.capacity > movie.soldTicket ? user.ticket * prices[movie.genre] : 0,
    }));
}
