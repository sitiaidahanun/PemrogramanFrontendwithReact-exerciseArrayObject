function gameGrouping(games) {
    ...
    games.forEach(game => {
        ...
        // apakah genre sudah ada di objek
        if (!groupedGames[genre]) {
            // Jika belum ada
            groupedGames[genre] = { title: [] };
        }

        groupedGames[genre].title.push(title);
    });

    return groupedGames; // Kembalikan hasil akhir
}
